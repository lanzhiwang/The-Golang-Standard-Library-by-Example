...
file 实现了 Reader 接口
file.Read(p) 将文件中的内容读入到 p 中  file -> p
```
...
file 实现了 Writer 接口
file.Write(p) 将 p 中的数据写入 file   file <- p
```
...
```
n, err := os.File.Read(p []byte)
n, err := os.File.Write(p []byte)
```

- strings.Reader 实现了 io.Reader

```
n, err := strings.Reader.Read(p []byte)
```

- bufio.Reader/Writer 分别实现了 io.Reader 和 io.Writer

```
n, err := bufio.Reader.Read(p)
n, err := bufio.Writer.Write(p)
```

- bytes.Buffer 同时实现了 io.Reader 和 io.Writer

```
n, err := bytes.Buffer.Read(p)
n, err := bytes.Buffer.Write(p)
```

- bytes.Reader 实现了 io.Reader

```
n, err := bytes.Reader.Read(p)
```
...
除此之外，io 包本身也有这两个接口的实现类型。如：
	...
	n, err := PipeReader.Read(p)
	n, err := SectionReader.Read(p)
	
	n, err := PipeWriter.Write(p)
...
```go
reader := strings.NewReader("Go语言中文网")  // 实现了 ReaderAt 接口
p := make([]byte, 6)
...
```go
// os.Create("writeAt.txt") 操作返回一个 os.File 类型
// os.File 实现了 WriterAt 接口
...
```go
// os.Open("writeAt.txt") 操作返回一个 os.File 类型
// os.File 实现了 Reader 接口  file.Read(p)
...
// bufio.NewWriter(os.Stdout) 实现了 ReaderFrom 接口
writer := bufio.NewWriter(os.Stdout)
writer.ReadFrom(file)  // file.Read(p) os.Stdout.Write(p)
writer.Flush()
```
...
```
n, err := bufio.Writer.ReadFrom(os.File)  // 文件中的内容存在哪个变量里了？ 和 bufio.Write 定义有关
n, err := strings.Buffe.ReadFrom(os.File)
```
...
n, err := bytes.NewReader.WriteTo(os.File)
// 先从 bytes.NewReade 读出所有内容到临时变量中
// 然后将临时变量的内容写入文件中
```
